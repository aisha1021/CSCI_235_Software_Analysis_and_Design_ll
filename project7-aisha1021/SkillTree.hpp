...
struct Skill {
   ...
   Skill(int id, const std::string& name, const std::string& description, bool leveled)
    : id_(id), name_(name), description_(description), leveled_(leveled) {}
...
private:
    ...
    */
    void inorderTraversal(std::shared_ptr<BinaryNode<Skill>> node, std::vector<Skill>& sorted_skills) const;
    ...
    */
    std::shared_ptr<BinaryNode<Skill>> buildBalancedTree(const std::vector<Skill>& sorted_skills, int start, int end) const;
    ...
    */
    void preorderTraverse(std::shared_ptr<BinaryNode<Skill>> node, std::function<void(const Skill&)> visit) const;
