...
/**
  * @post: Stores the ALIEN Creatures of highest level in the cavern's alien stack, 
  *        in the order in which they appear in the Cavern.
  *        Empty the stack before beginning.
  *        If the highest level is 5 and there are 3 ALIEN creatures with level 5, 
  *        the one with the lowest index in items_ is at the bottom of the stack 
  *        and the one with the highest index in items_ is at the top of the stack, 
  *        with a total of 3 ALIEN Creatures on the stack.
*/
void Cavern::initializeAlienStack() {
    clearAlienStack();
    int highest_lvl = 0;

    for (int i = 0; i < getCurrentSize(); i++) { // find the highest level among ALIEN Creatures
        if (items_[i]->getCategory() == "ALIEN" && items_[i]->getLevel() > highest_lvl) {
            highest_lvl = items_[i]->getLevel();
        }
    }

    for (int i = 0; i < getCurrentSize(); i++) { // store ALIEN Creatures of the highest level in the alien stack
        if (items_[i]->getCategory() == "ALIEN" && items_[i]->getLevel() == highest_lvl) {
...
/**
  * @post: Stores the UNDEAD Creatures of highest level in the cavern's undead stack, 
  *        in the order in which they appear in the Cavern.
  *        Empty the stack before beginning.
  *        If the highest level is 5 and there are 3 UNDEAD creatures with level 5, 
  *        the one with the lowest index in items_ is at the bottom of the stack 
  *        and the one with the highest index in items_ is at the top of the stack, 
  *        with a total of 3 UNDEAD Creatures on the stack.
*/
void Cavern::initializeUndeadStack() {
    clearUndeadStack();
    int highest_lvl = 0;

    for (int i = 0; i < getCurrentSize(); i++) { // find the highest level among UNDEAD Creatures
        if (items_[i]->getCategory() == "UNDEAD" && items_[i]->getLevel() > highest_lvl) {
            highest_lvl = items_[i]->getLevel();
        }
    }

    for (int i = 0; i < getCurrentSize(); i++) { // store UNDEAD Creatures of the highest level in the undead stack
        if (items_[i]->getCategory() == "UNDEAD" && items_[i]->getLevel() == highest_lvl) {
...
/**
  * @post: Stores the MYSTICAL Creatures of highest level in the cavern's mystical stack, 
  *        in the order in which they appear in the Cavern.
  *        Empty the stack before beginning.
  *        If the highest level is 5 and there are 3 MYSTICAL creatures with level 5, 
  *        the one with the lowest index in items_ is at the bottom of the stack 
  *        and the one with the highest index in items_ is at the top of the stack, 
  *        with a total of 3 MYSTICAL Creatures on the stack.
*/
void Cavern::initializeMysticalStack() {
    clearMysticalStack();
    int highest_lvl = 0;

    for (int i = 0; i < getCurrentSize(); i++) { // find the highest level among MYSTICAL Creatures
        if (items_[i]->getCategory() == "MYSTICAL" && items_[i]->getLevel() > highest_lvl) {
            highest_lvl = items_[i]->getLevel();
        }
    }

    for (int i = 0; i < getCurrentSize(); i++) { // store MYSTICAL Creatures of the highest level in the mystical stack
        if (items_[i]->getCategory() == "MYSTICAL" && items_[i]->getLevel() == highest_lvl) {
...
/**
  * @post: Stores the UNKNOWN Creatures of highest level in the cavern's unknown stack, 
  *        in the order in which they appear in the Cavern.
  *        Empty the stack before beginning.
  *        If the highest level is 5 and there are 3 UNKNOWN creatures with level 5, 
  *        the one with the lowest index in items_ is at the bottom of the stack 
  *        and the one with the highest index in items_ is at the top of the stack, 
  *        with a total of 3 UNKNOWN Creatures on the stack.
*/
void Cavern::initializeUnknownStack() {
    clearUnknownStack();
    int highest_lvl = 0;

    for (int i = 0; i < getCurrentSize(); i++) { // find the highest level among UNKNOWN Creatures
        if (items_[i]->getCategory() == "UNKNOWN" && items_[i]->getLevel() > highest_lvl) {
            highest_lvl = items_[i]->getLevel();
        }
    }

    for (int i = 0; i < getCurrentSize(); i++) { // store UNKNOWN Creatures of the highest level in the unknown stack
        if (items_[i]->getCategory() == "UNKNOWN" && items_[i]->getLevel() == highest_lvl) {
...
std::stack<Creature*> Cavern::getAlienStack() const {
    return alien_stack_; // return copy of alien stack
}
...
std::stack<Creature*> Cavern::getUndeadStack() const {
    return undead_stack_; // return copy of undead stack
}
...
std::stack<Creature*> Cavern::getMysticalStack() const {
    return mystical_stack_; // return copy of mystical stack
}
...
std::stack<Creature*> Cavern::getUnknownStack() const {
    return unknown_stack_; // return copy of unknown stack
}

/**
  * @post: clears the stack of highest level Aliens in the cavern
*/ 
void Cavern::clearAlienStack() { // remove elements from the alien_stack_ until empty
...
*/
void Cavern::clearUndeadStack() { // remove elements from the undead_stack_ until empty
...
*/
void Cavern::clearMysticalStack() { // remove elements from the mystical_stack_ until empty
...
*/
void Cavern::clearUnknownStack() { // remove elements from the unknown_stack_ until empty
...
*/
void Cavern::setAttacks(const std::stack<Creature*> creature_stack) {
    std::stack<Creature*> temp_stack = creature_stack; // create temporary stack

    if (!temp_stack.empty()) { // check if the input stack is not empty
        Creature* top_creature = temp_stack.top(); // get the top creature from the stack
        temp_stack.pop();

        if (top_creature->getCategory() == "ALIEN") { // clear the corresponding stack based on the top creature's category
            clearAlienStack();
        } else if (top_creature->getCategory() == "UNDEAD") {
            clearUndeadStack();
        } else if (top_creature->getCategory() == "MYSTICAL") {
            clearMysticalStack();
        } else if (top_creature->getCategory() == "UNKNOWN") {
            clearUnknownStack();
        }

        temp_stack.push(top_creature); // re-push the top creature into the temporary stack
    }

    std::stack<Creature*> reversed_stack; // create stack to add reversed creatures
    
    while (!temp_stack.empty()) { // loop through the temporary stack
        Creature* creature = temp_stack.top();
        temp_stack.pop();

        std::cout << "SELECT 2 ATTACKS FOR " << creature->getName() << std::endl; // prompt user to select attacks for the creature

        for (int i = 0; i < 2; i++) { // loop twice to select 2 attacks
            int attack_choice;

            creature->displayAttacks(); // display available attacks for the creature

            while (true) { // prompt user for attack choice and validate input
            ...
            creature->addAttack(attack_choice); // add the selected attack to the creature's attack queue
        }

        reversed_stack.push(creature); // push the creature into the reversed stack
    }

    while (!reversed_stack.empty()) { // rebuild the Cavern's appropriate stack based on the creatures' categories
        Creature* creature = reversed_stack.top();
        reversed_stack.pop();
