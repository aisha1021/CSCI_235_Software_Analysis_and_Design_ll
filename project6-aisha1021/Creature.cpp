...
std::queue<Attack> Creature::getAttackQueue() const {
    return attack_queue_; // return copy of attack queue
}
...
void Creature::addAttack(const Attack& attack) {
    attack_queue_.push(attack); // add attack to attack push
}
...
void Creature::clearAttackQueue() {
    while (!attack_queue_.empty()) {  // remove attacks from the attack queue until empty
        attack_queue_.pop();
    }
}